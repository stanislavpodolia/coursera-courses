...
class Assignment
  ...
  def create_todolist(params)
    TodoList.new(list_name: params[:name], list_due_date: params[:due_date]).save
  ...
  def find_allusers(offset, limit)
    User.all.offset(offset).limit(limit).order(:updated_at).to_a
  ...
  def find_alllists(offset, limit)
    TodoList.all.offset(offset).limit(limit).order(list_due_date: :desc).to_a
      # accept offset and limit input parameters
